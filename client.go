...
func (c *Client) do(req *Request) (*Response, error) {
	...
	for _, method := range methods {
		...
		if hresp, err := c.HTTPClient.Do(req.HTTPRequest); err != nil {
		...
		} else if hresp.StatusCode != 200 && hresp.StatusCode != 206 {
			// allow caller to see response if status code is not 200 or 206
			...
			if hresp.ContentLength > 0 {
				...
				resp.Size = resp.BytesTransferred() + uint64(0)
			}
